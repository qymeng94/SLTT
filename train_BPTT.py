...
def main():
    ...
    # output setting
    ##########################################################
    out_dir = os.path.join(args.out_dir, f'BPTT_{args.dataset}_{args.model}_{args.name}_T{args.T}_tau{args.tau}_e{args.epochs}_bs{args.b}_{args.opt}_lr{args.lr}_wd{args.weight_decay}_SG_{args.surrogate}_drop{args.drop_rate}_losslamb{args.loss_lambda}_')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for frame, label in train_data_loader:
            ...
            label = label.cuda()
            label_real = torch.cat([label for _ in range(args.T)], 0)
            ...
            if args.amp:
                with amp.autocast():
                    ...
                    if args.loss_lambda > 0.0:  # the loss is a cross entropy term plus a mse term
                        ...
                        loss = ((1 - args.loss_lambda) * F.cross_entropy(out_all, label_real) + args.loss_lambda * mse_loss)
                    else:  # the loss is just a cross entropy term
                        loss = F.cross_entropy(out_all, label_real)
                scaler.scale(loss).backward()

            else:
                raise NotImplementedError('We do not implement mixed precision training for BPTT. Please implement it by yourself.')
